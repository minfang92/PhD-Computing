...
########################################################
#Load data
inv <- read.csv(file = "InvestmentRate_0mu.csv") 
...
p <- ggplot(inv,aes(x=z,y=k,fill=valuefactor)) + 
  ...
  labs(x="Productivity Grid",y="Capital Stock Grid",title="Investment Rate at Steady State (0mu)")+
...
p
#export figure
ggsave(p,filename="SS_Investment_Policy_over_Distribution_0mu.eps",path = "00.Output", height=5.5,width=8.8,units="in",dpi=200)
...
########################################################
#Load data
prob <- read.csv(file = "Prob_of_Adjustment_0mu.csv") 
...
p <- ggplot(prob,aes(x=z,y=k,fill=valuefactor)) + 
  ...
  labs(x="Productivity Grid",y="Capital Stock Grid",title="Adjustment Probability at Steady State (0mu)")+
  scale_fill_manual(values=c("#ef6548","#d7301f","#990000"))+
  #coord_fixed()+
  theme_grey(base_size=15)+
  theme(legend.position="right",legend.direction="vertical",
        legend.title=element_text(colour=textcol),
...
p
#export figure
ggsave(p,filename="SS_Adjustment_Policy_over_Distribution_0mu.eps",path = "00.Output", height=5.5,width=8.8,units="in",dpi=200)
