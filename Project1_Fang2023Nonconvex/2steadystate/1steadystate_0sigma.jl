...
p = Param(;β = 0.99,
           ...
           ν = 0.60,
           S = 0.00,
           a = 0.004,  #needs some larger size to converge
           ϕ_k = 0.00,
           σz = 0.05,
           μξ = ξbar/2,
           σξ = 0.00001
...
CSV.write("2steadystate/InvestmentRate_0sigma.csv", data)
...
CSV.write("2steadystate/Prob_of_Adjustment_0sigma.csv", data)
...
### Save Results
CSV.write("2steadystate/transition_0sigma.csv", tr_output)
# Readback
tr_output = CSV.read("2steadystate/transition_0sigma.csv",  copycols = true, typemap = Dict(Int64=>Float64))
