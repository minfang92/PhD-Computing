...
!!!
CSV.write("1calibration/02.Identification_results/Identification_df_Auto3.csv", Identification_df)

# Firstload dataframe
Identification_df = CSV.read("1calibration/02.Identification_results/Identification_df_Auto3.csv", copycols = true, typemap = Dict(Int64=>Float64))


# Function
function Identification_df_Auto3(α = 0.25,ν = 0.60; #KEY
                          ϕk_lower = 0.0,
                          ϕk_upper = 2.4,
                          sigmaxi_lower = 0.0,
        ...
        for iϕk in 0:1
            for iξ in 1:ξ_number
                    ...
                    sigmaxi = sigmaxi_lower + (sigmaxi_upper-sigmaxi_lower)/ξ_number*iξ
                    #Params
                    p = Param(α = α, ν = ν; μξ = ξbar/2, σξ = sigmaxi, ϕ_k = ϕ_k)
                    ...
                    results = (alpha=p.α, nu=p.ν, phi_k=p.ϕ_k, sigmaxi=sigmaxi, a=p.a, wss=wss, els=els)
